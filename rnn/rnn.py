...
def produce_text(initial, sess, pred, length=sample_length):
    ...
    for i in range(length):
        ...
        #pred_a = sess.run(pred, feed_dict={x: a})
        prob = tf.nn.softmax(pred[0])
        prob = sess.run(prob, feed_dict={x: a})
        decision = np.random.choice(range(vocab_size), p=(prob/np.sum(prob)))
        #print(pred_a)
